...
contract DAOTokenCreationProxyTransferer {
    ...
    function sendValues() {
     ...
     // Cast DAO to TokenCreation contract
        TokenCreation fueling = TokenCreation(dao);

        // Check if the TokenCreation contract implements the closingTime function
        if (address(fueling).call(bytes4(keccak256("closingTime()"))) && 
            now > fueling.closingTime() ||
            !fueling.createTokenProxy.value(this.balance)(owner)) {
